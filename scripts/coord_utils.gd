...
static func get_random_pos(start_pos: Vector2, end_pos: Vector2, units: Array[UnitBase] = [], limit := 100) -> Vector2i:
	...
	for i in range(limit):
		...
		if i < limit - 1 and Helper.some(units, func(item: UnitBase): return abs(item.global_position.distance_to(vec)) <= UnitBase.UNIT_RADIUS):
			continue
		return vec
